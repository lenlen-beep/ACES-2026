...
# --------------------------------------------------
# Linearer Klimatrend (Rolling-Jahresmittel)
# --------------------------------------------------
...
# Residuum Zeitreihe
axes[1, 0].plot(t, T_residuum, color='gray', linewidth=0.4, alpha=0.8)
axes[1, 0].axhline(0, color='black', linewidth=0.8)
axes[1, 0].set_xlabel("Stunde")
axes[1, 0].set_ylabel("Residuum [°C]")
axes[1, 0].set_title(f"Residuum nach Zerlegung\nStd = {T_residuum.std():.2f}°C")
...
# AR(1)-Simulation: X_t = γ·X_{t-1} + σ·ε_t
sigma_stationaer = sigma_ou / np.sqrt(1 - gamma**2)
T_ou_sim = np.zeros(n)
T_ou_sim[0] = np.random.normal(0, sigma_stationaer)
for i in range(1, n):
    T_ou_sim[i] = gamma * T_ou_sim[i-1] + sigma_ou * np.random.normal()
...
def simulate_temperature_mc(T_jahresgang_year, T_tagesgang_year,
    ...
    for s in range(n_sim):
        ...
        for i in range(1, n_h):
            ou[i] = gamma * ou[i-1] + sigma_ou * noise[i]
...
T_mc = simulate_temperature_mc(T_jg_param, T_tg_param, gamma, sigma_ou,
...
ax.set_title(f"MC-Simulation Temperatur (n={T_mc.shape[0]}) + historische Jahre\n"
             f"γ={gamma:.6f}, σ={sigma_ou:.4f}°C/√h, "
